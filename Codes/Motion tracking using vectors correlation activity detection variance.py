...
targetDirectory =  "_path_to_exercise_videos_" + desired_activity
...
def process_video(videofile, sub_no):
        
    currFileName = os.path.basename(videofile).split('.')[0]
    currAct = currFileName.split('_')[1:]
    ...
    # cap = cv2.VideoCapture(0) # To get input from webcam 
    ...
    peak_detect_threshold = 0.8 # Adjust as required

    if desired_activity == 'run' or desired_activity == 'jump':
        alpha = 0.85  # Adjust for smoother motion tracking 
        min_distance = 2 
        peak_detect_threshold = 0.4
    
    else:
        alpha = 0.08  # Adjust for smoother motion tracking 
        min_distance = 10
    ...
    with mp_pose.Pose(min_detection_confidence=0.9, min_tracking_confidence=0.9) as pose:
        ...
        while cap.isOpened():
                        ...
                        if frame_count > 2 and wait_idx <=0:  
                            ...
                            if len(autocorr) > 20:
                                ...
                                if motion_amplitude > motion_amplitude_threshold:
                                    ...
                                    if repetition_count > prev_repetition_count:
                                        # Plot the motion
                                        plt.figure(figsize=(10, 5))
                                        plt.plot(motion_distance, marker='.', linestyle='-', color='b', label='Velocity')
                                        
                                        # Labels and title with font size
                                        plt.xlabel('Frame Index', fontsize=14)
                                        plt.ylabel('Velocity', fontsize=14)
                                        plt.title('Velocity vs Frame Indices', fontsize=16)
                                        plt.legend(fontsize=12)
                                        plt.grid(False)
                                        plt.xticks(fontsize=12)
                                        plt.yticks(fontsize=12)
                                        plt.savefig(save_img_path + '/velocity_plot.png', dpi=1200, bbox_inches='tight')
                                        plt.show()
                                        
                                        # Plot the autocorrelation for motion with detected peaks
                                        plt.figure(figsize=(10, 5))
                                        plt.plot(autocorr, marker='.', linestyle='-', color='b', label='Autocorrelation for the velocity')
                                        
                                        # Labels and title with font size
                                        plt.xlabel('Frame Index', fontsize=14)
                                        plt.ylabel('Autocorrelation', fontsize=14)
                                        plt.title('Autocorrelation vs Frame Indices', fontsize=16)
                                        plt.legend(fontsize=12)
                                        plt.grid(False)
                                        plt.xticks(fontsize=12)
                                        plt.yticks(fontsize=12)
                                        plt.savefig(save_img_path + '/autocorrelation_plot.png', dpi=1200, bbox_inches='tight')
                                        plt.show()
                                        ...
                                        plt.vlines(x=peaks, ymin=autocorr[peaks] - properties["prominences"],
                                                    ymax=autocorr[peaks], color="C1", label='Prominences')
                                        
                                        # Add labels, title, and legend
                                        plt.xlabel('Frame Index', fontsize=14)
                                        plt.ylabel('Autocorrelation', fontsize=14)
                                        plt.title('Detected Peaks in Autocorrelation', fontsize=16)
                                        plt.legend(fontsize=12)
                                        plt.xticks(fontsize=12)
                                        plt.yticks(fontsize=12)
                                        plt.savefig(save_img_path + '/autocorrelation_peaks_plot.png', dpi=1200, bbox_inches='tight')
                                        plt.show()
            ...
            frame = ResizeWithAspectRatio(frame, width = desiredFrameWidth)

            
        
            # # Display repetition count
            cv2.putText(frame, f"Repetitions: {repetition_count}", (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 
...
for file in mp4files:
    subject_number+=1
    process_video(file, subject_number)
...
# --- Mean Absolute Error ---
absolute_errors = [abs(p - g) for p, g in zip(counting_results, ground_truth)]
mae = sum(absolute_errors) / len(absolute_errors)

# --- Root Mean Squared Error ---
squared_errors = [(p - g) ** 2 for p, g in zip(counting_results, ground_truth)]
